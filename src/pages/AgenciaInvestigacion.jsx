...
const AgenciaInvestigacion = () => {
    ...
    const schema = {
      ...
      "url": `https://www.sdinvestigaciones.com${lang() === 'es' ? '' : '/' + lang()}/agencia-investigacion-privada-tenerife`
    };

    const faqSchema = {
      "@context": "https://schema.org",
      "@type": "FAQPage",
      "mainEntity": t().agencyPage.faq.items.map(item => ({
        "@type": "Question",
        "name": item.q,
        "acceptedAnswer": {
          "@type": "Answer",
    ...
    const breadcrumbSchema = {
      ...
      "itemListElement": [
        ...
        },
        {
          "@type": "ListItem",
          "position": 2,
          "name": t().agencyPage.h1,
          "item": `https://www.sdinvestigaciones.com${lang() === 'es' ? '' : '/' + lang()}/agencia-investigacion-privada-tenerife`
        }
      ]
    };
  ...
  return (
    <main class="seo-page">
      <Title>{t().agencyPage.seoTitle}</Title>
      <Meta name="description" content={t().agencyPage.seoDesc} />
      ...
      <section class="container section">
        <h1 class="text-gradient text-4xl font-bold mb-6 text-center">{t().agencyPage.h1}</h1>
        <p class="text-xl text-center text-gray-300 max-w-3xl mx-auto mb-12">{t().agencyPage.intro}</p>

        <div class="grid md:grid-cols-2 gap-8 mb-16">
           <div class="glass-card p-8 rounded-2xl">
               <h2 class="text-2xl font-bold mb-6 text-blue-400">{t().agencyPage.legalTitle}</h2>
               <ul class="space-y-4">
                   {t().agencyPage.legalList.map(item => (
           ...
           <div class="glass-card p-8 rounded-2xl">
               <h2 class="text-2xl font-bold mb-6 text-blue-400">{t().agencyPage.expTitle}</h2>
               <p class="text-gray-300 leading-relaxed text-lg">{t().agencyPage.expText}</p>
           </div>
        </div>
        
        <article class="glass-card p-8 rounded-2xl mb-16 content-block">
            <h2 class="text-3xl font-bold mb-6 text-white">{t().agencyPage.content.title}</h2>
            <div class="prose prose-invert max-w-none text-gray-300 space-y-4">
                <p innerHTML={t().agencyPage.content.p1}></p>
                <p innerHTML={t().agencyPage.content.p2}></p>
            </div>
        </article>

        <section class="faq-section max-w-3xl mx-auto mb-16">
            <h2 class="text-3xl font-bold mb-8 text-center">{t().agencyPage.faq.title}</h2>
             <div class="space-y-4">
              {t().agencyPage.faq.items.map(item => (
        ...
        <div class="cta-box text-center glass-card p-12 rounded-2xl mb-16 bg-gradient-to-br from-blue-900/30 to-transparent">
          <h3 class="text-3xl font-bold mb-6">{t().agencyPage.cta.title}</h3>
...
export default AgenciaInvestigacion;
